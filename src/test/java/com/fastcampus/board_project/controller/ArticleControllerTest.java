...
class ArticleControllerTest {
  ...
  void givenPagingAndSortingParams_whenSearchingArticlesPage_thenReturnsArticlesPage() throws Exception {
    // Given
    String sortName = "title"; // 정렬 관련 파라미터
    String direction = "desc";
    int pageNumber = 0;
    int pageSize = 5;
    Pageable pageable = PageRequest.of(pageNumber, pageSize, Sort.by(Sort.Order.desc(sortName))); // 페이징 파라미터
    ...
    mvc.perform(
                    get("/articles")
                            .queryParam("page", String.valueOf(pageNumber)) // .queryParam 으로 위의 파라미터들을 받는다.
            ...
            .andExpect(view().name("articles/index"))
            .andExpect(model().attributeExists("articles"))
            .andExpect(model().attribute("paginationBarNumbers", barNumbers)); // 실제로 페이징 barNumber가 예상한 barNumber랑 같은가
    then(articleService).should().searchArticles(null, null, pageable);
    then(paginationService).should().getPaginationBarNumbers(pageable.getPageNumber(), Page.empty().getTotalPages()); // given & when 쪽의 검증
  }
  ...
  @WithUserDetails(value = "TestAuthor", setupBefore = TestExecutionEvent.TEST_EXECUTION)
